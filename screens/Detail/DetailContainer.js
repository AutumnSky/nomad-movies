...
export default class extends React.Component {
  ...
  constructor(props) {
    super(props);
    const {
      navigation: { state: { params: { isMovie, id, posterPhoto, backgroundPhoto, title, voteAvg, overview } } }
    } = props;
    this.state = {
      isMovie,
      id,
  ...
  async componentDidMount() {
    ...
    try {
      if (isMovie) {
        ...
        ({
          data: { genres, overview, status, first_air_date: date, title: name, backdrop_path: backgroundPhoto }
        } = await tv.getShow(id));
      }
    } catch (error) {
      console.log(error);
